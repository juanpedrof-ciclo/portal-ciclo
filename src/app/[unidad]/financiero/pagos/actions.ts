...
import type { DestinoPago, TipoPago, Unidad } from "@/lib/financiero/types";
...
export async function crearPago(
  ...
  const { data: pago, error: errorPago } = await supabase
    .from("pagos")
    .insert({ unidad, tipo, fecha, monto, destino, referencia, notas })
    .select("id")
    .single();
...
export async function anularPagosLote(
  unidad: Unidad,
  ids: string[],
