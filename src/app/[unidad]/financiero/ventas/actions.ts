...
import type { Canal, MapeoColumnas, Unidad } from "@/lib/financiero/types";
...
export async function procesarCargaVentas(
  ...
  if (guardarFormato) {
    const { data: creado, error: errorFormato } = await supabase
      .from("formatos_carga")
      .insert({ unidad, nombre: nombreFormato, mapeo_columnas: mapeo })
      .select("id")
      .single();
  ...
  for (const pedido of pedidosAgrupados) {
    ...
    try {
      cliente_id = await obtenerOCrearCliente(supabase, unidad, pedido.cliente, pedido.telefono);
    } catch (err) {
  ...
  const { error: errorPedidos } = await supabase
    .from("pedidos")
    .upsert(filasPedidos, { onConflict: "unidad,plataforma,id_orden_externo" });
  ...
  for (const { fecha, canal: canalGrupo } of fechasCanalTocados.values()) {
    try {
      await recalcularIngresoPedidos(supabase, unidad, fecha, canalGrupo);
    } catch (err) {
      return {
  ...
  revalidatePath(`/${unidad}/financiero/ingresos`);
...
export async function anularPedido(
  ...
  try {
    await recalcularIngresoPedidos(supabase, unidad, pedido.fecha, pedido.canal);
  } catch (err) {
    return {
...
export async function anularPedidosLote(
  unidad: Unidad,
  ids: string[],
  ...
  for (const { fecha, canal } of combos.values()) {
    try {
      await recalcularIngresoPedidos(supabase, unidad, fecha, canal);
    } catch (err) {
      advertencias.push(
