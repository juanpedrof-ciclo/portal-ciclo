...
import type { Cliente, Unidad } from "@/lib/financiero/types";
import { formatFechaCorta } from "@/lib/financiero/types";

export const metadata = { title: "Clientes · Módulo Financiero" };
...
export default async function ClientesPage({
  ...
  const RUTA = `/${unidad}/financiero/clientes`;
                ...
                clientes.map((c) => (
                  <tr key={c.id}>
                    <td className="px-4 py-3 text-zinc-900 dark:text-zinc-100">
                      <Link
                        href={`/${unidad}/financiero/resultados/cartera-clientes/${c.id}`}
