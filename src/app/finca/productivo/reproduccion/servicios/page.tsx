...
import { anularServicio, anularServiciosLote, crearServicio } from "./actions";
...
import { TIPO_SERVICIO_LABELS, type ServicioReproductivo, type Reproductor } from "@/lib/productivo/types";

export const metadata = { title: "Servicios · Módulo Productivo" };

const RUTA = "/finca/productivo/reproduccion/servicios";
const COLUMNAS_ORDEN = ["fecha", "tipo"] as const;
...
export default async function ServiciosPage({
  ...
  const [animales, mapaAnimales, { data: reproductores }] = await Promise.all([
    obtenerAnimalesActivos(supabase),
    obtenerMapaAnimales(supabase),
    supabase.from("reproductores").select("*").order("nombre").returns<Reproductor[]>(),
  ]);
  ...
  const { data: registros, count } = await query
    ...
    .range(desde, hasta)
    .returns<ServicioReproductivo[]>();

  const idsVisibles = (registros ?? []).map((r) => r.id);

  return (
    <div className="flex flex-col gap-6">
      <ServicioForm animales={animales} reproductores={reproductores ?? []} action={crearServicio} />
      ...
      <SeleccionProvider idsVisibles={idsVisibles}>
        <AnularSeleccionadosBar idsVisibles={idsVisibles} action={anularServiciosLote} />
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-zinc-200 text-xs uppercase tracking-wide text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium">Reproductor</th>
                ...
                {!registros || registros.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-6 text-center text-zinc-500 dark:text-zinc-400">
                      {q ? "No se encontraron servicios para esa búsqueda." : "Aún no hay servicios registrados."}
                    </td>
                  </tr>
                ) : (
                  registros.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-300">
                        {TIPO_SERVICIO_LABELS[r.tipo]}
                      </td>
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-300">
                        {r.reproductor_id ? reproductoresPorId[r.reproductor_id] ?? "—" : "—"}
                      </td>
                      <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400">{r.notas ?? "—"}</td>
                      <td className="px-4 py-3">
                        <AnularForm
                          id={r.id}
                          action={anularServicio}
                          mensaje="¿Seguro que deseas anular este servicio?"
                        />
                      </td>
                    </tr>
                  ))
