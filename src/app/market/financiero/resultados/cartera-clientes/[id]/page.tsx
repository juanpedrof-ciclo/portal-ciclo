...
import type { Cliente, VistaPedidoSaldo } from "@/lib/financiero/types";
import { formatCOP, formatFechaCorta } from "@/lib/financiero/types";

export const metadata = { title: "Cartera del cliente · Módulo Financiero · Ciclo Market" };
...
export default async function CarteraClientePage({
  ...
  const [{ data: cliente }, { data: pedidos }] = await Promise.all([
    supabase.from("clientes").select("*").eq("id", id).returns<Cliente[]>().single(),
    supabase
  ...
  return (
    <div className="flex flex-col gap-4">
      <BackLink
        href="/market/financiero/resultados/cartera-clientes"
